...
export async function uploadFile(filePath) {
  ...
  try {
    console.log(`Connecting to MongoDB Atlas to upload ${fileName}...`);
    ...
    if (fileName === 'master_post.json') {
      ...
      console.log(`✓ Updated master_offices (${docs.length} rows)`);
      ...
      await coll.deleteMany({});
      await coll.insertMany(docs);
      console.log(`✓ Updated master_services (${docs.length} rows)`);
      ...
      console.log(`✓ Upserted target file [${fileId}]: ${docs.length} rows`);
    } else {
      // Actual transaction file
      const match = fileName.match(/^(.+?)_(\d{4})_(\d+)\.json$/);
      if (!match) {
        throw new Error(`Unrecognized file pattern: ${fileName}`);
      }
      ...
      await coll.deleteMany({ file_identifier: fileId });
      if (docs.length > 0) await coll.insertMany(docs);
      console.log(`✓ Upserted transaction file [${fileId}] (${sourceType}): ${docs.length} rows`);
    }
  } finally {
    await client.close();
  }
...
if (process.argv[2]) {
  uploadFile(process.argv[2]).catch((err) => {
